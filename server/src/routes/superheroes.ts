...
router.post('/', upload.array('images', 5), async (req: MulterRequest, res: Response) => {
  try {
...
router.put('/:id', upload.array('images', 5), async (req: MulterRequest, res: Response) => {
  try {
    const hero = await Superhero.findById(req.params.id);
    if (!hero) return res.status(404).json({ error: 'Not found' });
...
router.delete('/:id/image/:publicId', async (req: Request, res: Response) => {
  try {
    const hero = await Superhero.findById(req.params.id);
